...
import {
  Activity, RotateCcw, CheckCircle, AlertTriangle, ArrowDown, Footprints, Camera
} from 'lucide-react';
...
export default function FitCoinPoseDetector({ exercise, detectFn, onComplete, onFeedback }) {
  const webcamRef = useRef(null);
  const canvasRef = useRef(null);
  const [count, setCount] = useState(0);
  const [angle, setAngle] = useState(0);
  const [status, setStatus] = useState(exercise.initialState);
  const [loading, setLoading] = useState(true);
  const [feedbackKey, setFeedbackKey] = useState('no_pose');

  const statusRef = useRef(exercise.initialState);
  const countRef = useRef(0);
  const completedRef = useRef(false);
  const throttleRef = useRef(0);

  const syncStatus = (val) => { statusRef.current = val; setStatus(val); };
  const syncCount = useCallback((updater) => {
    setCount((prev) => {
  ...
  return (
    <div>
      ...
      {/* Form Feedback (스쿼트 등 hasFeedback 운동만) */}
      {exercise.hasFeedback && <FormFeedback feedbackKey={feedbackKey} />}
